...
$('#submit').bind('click', function(){
	...
	if(getproject_notes == ""){ 
		$('#project_notes').css('border', '3px solid yellow');
		return false;
	}
	if(getclient_phone == ""){
		$('#client_phone').css('border', '3px solid yellow');
		return false;
	}
	if(getclient_email == ""){ 
		$('#client_email').css('border', '3px solid yellow');
		return false;
	}else{
		...
		*/	
		...
		projectObject.save({getproject_name: "project_name"},{project_url: "getproject_url"},{project_notes: "getproject_notes"},{client_phone: "getclient_phone"},{client_email: "getclient_email"}, 
		  ...
		  error: function(projectObject, error) {
			...
			alert('Failed to create new projectObject, with error code: ' + error.description);
		  }
		});
	}	
});
